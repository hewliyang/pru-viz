...
def census():
    ...
    with col1:
        ...
        selected_geo_filter = st.selectbox(
            "Select constituency level", ["DUN", "Parlimen", "State"], index=2
        )
        ...
        if selected_geo_filter:
            ...
            if selected_geo_filter:
                if selected_geo_filter == "DUN":
                    ...
                    filtered_parlimen = parlimen_gdf[
                        parlimen_gdf["state"] == selected_state
                    ]
                ...
                if selected_geo_filter == "State":
                    ...
                    selected_area = st.selectbox(
                        "Select area",
                        sorted(list(filtered_dun.dun.unique()))
                        if selected_geo_filter == "DUN"
                        else sorted(list(filtered_parlimen.parlimen.unique()))
                        if selected_geo_filter == "Parlimen"
                        else list(),
                    )
    ...
    with col2:
        ...
        if not filter_flag:
            ...
            selected_area_df = (
                dun_gdf[dun_gdf["dun"] == selected_area]
                if selected_geo_filter == "DUN"
                else parlimen_gdf[parlimen_gdf["parlimen"] == selected_area]
                if selected_geo_filter == "Parlimen"
                else state_gdf[state_gdf["state"] == selected_state]
            )
    ...
    if filter_flag:
        ...
        if selected_geo_filter == "State":  # state case
            census_parlimen_df = census_parlimen_df.query(
                f'state == "{selected_state}"'
            )  # operate on copy cuz we need the original df later
            census_district_df = census_district_df.query(
                f'state == "{selected_state}"'
            )

            pie_array_ns(
                census_district_df=census_district_df,
                census_parlimen_df=census_parlimen_df,
            )

        elif selected_geo_filter == "Parlimen":  # parlimen case
            census_parlimen_df = census_parlimen_df.query(
                f'parlimen == "{selected_area}"'
            )
        ...
        pie_array_ns(
            census_district_df=census_district_df, census_parlimen_df=census_parlimen_df
        )
    ...
    to_highlight = st.multiselect(
        label="Choose states to highlight",
        options=states,
    ...
    try:
        c1.altair_chart(
            make_altair_bar(
                census_parlimen_df_cp, selections[0], to_highlight=to_highlight
            ),
            use_container_width=True,
        )
        c2.altair_chart(
            make_altair_bar(
                census_parlimen_df_cp, selections[1], to_highlight=to_highlight
            ),
            use_container_width=True,
        )
        c3.altair_chart(
            make_altair_bar(
                census_parlimen_df_cp, selections[2], to_highlight=to_highlight
            ),
            use_container_width=True,
        )
        c4.altair_chart(
            make_altair_bar(
                census_parlimen_df_cp, selections[3], to_highlight=to_highlight
            ),
            use_container_width=True,
        )
    except:
